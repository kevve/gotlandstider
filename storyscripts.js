...
function getSocialButtons(igUrl, ttUrl, isDarkMode = false) {
    ...
    return `
    <div class="flex flex-row gap-4 w-full justify-center mt-6 flex-wrap">
        <!-- Instagram -->
        <a href="${igUrl}" target="_blank" class="group relative flex items-center justify-center gap-3 px-6 py-3 rounded-xl border border-white/40 bg-white/20 backdrop-blur-md hover:bg-white/40 hover:border-white/60 transition-all duration-300 w-40">
        ...
        <!-- TikTok -->
        <a href="${ttUrl}" target="_blank" class="group relative flex items-center justify-center gap-3 px-6 py-3 rounded-xl border border-white/40 bg-white/20 backdrop-blur-md hover:bg-white/40 hover:border-white/60 transition-all duration-300 w-40">
            <svg class="w-5 h-5 ${iconColor} group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24">
        ...
        html = `
            <div class="w-full h-[80vh] md:h-[90vh] bg-black flex justify-center items-center">
                <div class="relative aspect-[9/16] h-full w-full max-w-sm md:max-w-none">
                    <video class="w-full h-full object-cover" controls autoplay playsinline poster="${story.poster}">
    ...
    // --- CASE 2: VIDEO + TEXT ---
        ...
        html = `
            ...
            <div class="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-gotland-stone">
                ...
                <h2 class="font-serif text-3xl md:text-4xl text-gotland-deep mb-6">${story.title}</h2>
                <p class="text-gray-700 leading-relaxed mb-8">${story.text}</p>
...
// --- 5. DEEP LINKING (Lyssna vid sidladdning) ---
